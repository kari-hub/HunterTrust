...
function trackUserBehavior(senderEmail) {
  var cache = CacheService.getUserCache();
  var senderBehavior = cache.get(senderEmail) || {frequency: 0, recipients: []};
  
  senderBehavior.frequency += 1;
  senderBehavior.lastSent = new Date();
  
  // Example: add recipient tracking
  var recipients = message.getTo().split(',');
  senderBehavior.recipients.push(...recipients);
  
  cache.put(senderEmail, JSON.stringify(senderBehavior), 21600); // Cache for 6 hours
  
  return senderBehavior;
}

var senderBehavior = trackUserBehavior(message.getFrom());
if (senderBehavior.frequency > 5) {
  Logger.log("High frequency detected for sender: " + message.getFrom());
}
